...
#  initial hack collection of commands to get an end result... optimization and cleanup pending

# make dimansions consistent
mkdir output
for i in *.jpg; do      time=$(echo $i | cut -d . -f 1);     ffmpeg -nostdin -y -i $i -vf "scale=1920:1080:force_original_aspect_ratio=>

cd output
# embed filename in image (removing '_001' from name)
mkdir raw
for i in *.jpg; do      time=$(echo $i | cut -d . -f 1 | rev | cut -c 5- | rev )'(EST)';     ffmpeg -nostdin -y -i $i -vf "drawtext=tex>
